...
def load_pool(data_dir: Path = DATA_DIR, standard_only: bool = True) -> list[Card]:
    """全セットを読み込み、重複排除済みのカードプールを返す。"""
    ...
    for path in _iter_set_files(data_dir):
        ...
        for c in data.get("cards", []):
            ...
            if types & EXCLUDE_TYPES or BASIC_SUPERTYPE in supertypes:
                continue
            legal = c.get("legalities", {})
            if standard_only and legal.get("standard") != "Legal":
                continue
            by_name[name] = Card(
                name=name,
